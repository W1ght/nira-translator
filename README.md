...
- 页面翻译：优先翻译当前视口附近的正文，滚动后继续按需请求；支持动态内容、开放式 Shadow DOM 与 iframe。
- 双语对照 / 仅译文：切换显示方式不会重复调用模型，关闭翻译会恢复页面。
- 页面悬浮球：网页右下角提供纯黑白快捷入口，单击即可开启或关闭整页翻译。
- 划词翻译：浮动按钮、`Alt + Shift + T` 快捷键、加载态、轻微呼吸微光、复制与重试。
- 模型管理：支持 KISS Translator 同类翻译服务；页面翻译与划词翻译可独立选择模型。
...
- 可翻译开放式 Shadow DOM 和允许扩展注入的 iframe；网页主动关闭的 Shadow DOM 无法从外部访问。
- 译文会复用原文已有的安全结构以保留链接、强调和行内代码，但不会把模型输出作为任意 HTML 注入。
